// routes/categories.js - Rutas de categorías para ATHENA BRAND
...
router.get('/', (req, res) => {
  try {
    ...
    const activeCategories = athenaCategories
...
router.get('/menu', (req, res) => {
  try {
    const menuCategories = athenaCategories
      .filter(cat => cat.isActive)
      .sort((a, b) => a.order - b.order)
      .map(cat => ({
        slug: cat.slug,
...
router.get('/:slug', (req, res) => {
  try {
    ...
    const category = athenaCategories.find(cat => 
...
router.get('/admin/all', adminAuth, (req, res) => {
  try {
    res.json({
      success: true,
      data: athenaCategories.sort((a, b) => a.order - b.order),
...
router.get('/sitemap', (req, res) => {
  try {
    const sitemapData = athenaCategories
      .filter(cat => cat.isActive)
      .map(cat => ({
        slug: cat.slug,
        name: cat.name,
